#!/usr/bin/env bash
# Appends the newest (or an explicit) parallel-web-tools release to releases.json
# as a new version-table entry (keyed by version) and sets .latest to it.
# Existing entries are preserved so consumers can still select past versions.
#
# parallel-web-tools ships tagged GitHub releases with per-arch prebuilt zips, so:
#   key     = version (tag without the leading "v")
#   .hashes = per-system SRI hash of the release zip (fetchzip / --unpack)
#
...
# nix system -> release-asset arch name
...
verify_build() {
  ...
  local out_path
  if ! out_path="$(cd "$pkg_dir" && nix build ".#parallel-cli_${sanitized_key}" --no-write-lock-file --no-link --print-out-paths)"; then
    log_error "nix build failed for parallel-cli_${sanitized_key}"
    return 1
  fi
  ...
  timeout 30 "$out_path/bin/parallel-cli" --help >/dev/null 2>&1 || true
...
Examples:
  ...
  ./scripts/update-version.sh --version 0.7.1
EOF
}
...
main() {
  ...
  # Compute per-arch SRI hashes from the release zips.
  log_info "Prefetching zip hashes..."
  local system arch url sri_hash
  local hashes_json="{}"
  for system in "${!SYSTEM_TO_ARCH[@]}"; do
    arch="${SYSTEM_TO_ARCH[$system]}"
    url="https://github.com/$REPO_OWNER/$REPO_NAME/releases/download/$latest_tag/parallel-cli-${arch}.zip"
    sri_hash="$(prefetch_sha256_sri "$url")" || { log_error "Failed to prefetch $system ($arch) zip hash"; exit 2; }
    if [ -z "$sri_hash" ]; then
      log_error "Failed to prefetch $system ($arch) zip hash"
      exit 2
    fi
  ...
  log_info "Successfully appended parallel-web-tools $latest_version (latest was $current_version)"
}
