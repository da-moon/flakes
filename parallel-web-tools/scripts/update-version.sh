...
verify_build() {
  ...
  local out_path
  if ! out_path="$(cd "$pkg_dir" && nix build .#parallel-cli --no-write-lock-file --no-link --print-out-paths)"; then
...
main() {
  ...
  hash_aarch64="$(prefetch_sha256_sri "$url_aarch64")" || { log_error "Failed to prefetch aarch64 zip hash"; exit 2; }
  hash_x86_64="$(prefetch_sha256_sri "$url_x86_64")" || { log_error "Failed to prefetch x86_64 zip hash"; exit 2; }
