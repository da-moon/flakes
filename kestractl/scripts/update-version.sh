#!/usr/bin/env bash
# Appends the newest (or an explicit) kestra-io/kestractl release to releases.json
...
#
# kestractl ships TAGGED GitHub releases with per-arch prebuilt raw binaries:
#   key     = the release version (tag with the leading "v" stripped)
#   version = same
# Per-arch fixed-output hashes are prefetched from the release assets and
...
readonly REPO_OWNER="kestra-io"
readonly REPO_NAME="kestractl"

# Map nix system -> upstream release-asset OS/arch tokens.
...
asset_url() {
  local version="$1"
  local suffix="$2"
  printf 'https://github.com/%s/%s/releases/download/v%s/kestractl_%s_%s\n' \
    "$REPO_OWNER" "$REPO_NAME" "$version" "$version" "$suffix"
}
...
verify_build() {
  ...
  local out_path
  if ! out_path="$(cd "$pkg_dir" && nix build ".#kestractl_${sanitized_key}" --no-link --print-out-paths --no-write-lock-file)"; then
    log_error "nix build failed for kestractl_${sanitized_key}"
    return 1
  fi
  if [ -z "$out_path" ] || [ ! -x "$out_path/bin/kestractl" ]; then
    log_error "Build succeeded but expected binary not found at: $out_path/bin/kestractl"
    return 1
  fi
  ...
  timeout 10 "$out_path/bin/kestractl" --help >/dev/null 2>&1 || true
...
Appends the newest (or an explicit) kestractl release to releases.json as a new
version-table entry (keyed by version) and sets it as .latest. Existing entries
...
Examples:
  ...
  ./scripts/update-version.sh --version 1.18.1
EOF
}
...
main() {
  ...
  # Prefetch per-arch binary hashes from the release assets.
  ...
  for system in "${!SYSTEM_TO_RELEASE_SUFFIX[@]}"; do
    ...
    url="$(asset_url "$latest_version" "$suffix")"
  ...
  log_info "Successfully appended kestractl $latest_version (latest was $current_version)"
}
