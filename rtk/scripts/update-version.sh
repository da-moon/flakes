...
readonly REPO_OWNER="rtk-ai"
readonly REPO_NAME="rtk"
...
get_release_checksums() {
  local tag="$1"
  curl -fsSL "https://github.com/$REPO_OWNER/$REPO_NAME/releases/download/$tag/checksums.txt"
}
...
verify_build() {
  ...
  local out_path
  if ! out_path="$(cd "$pkg_dir" && nix build .#rtk --no-link --print-out-paths)"; then
  ...
  "$out_path/bin/rtk" --version >/dev/null 2>&1 || true
...
Examples:
  ...
  ./scripts/update-version.sh --version 0.37.2
EOF
}
...
main() {
  ...
  local checksums
  checksums="$(get_release_checksums "$latest_tag")"
  if [ -z "$checksums" ]; then
    log_error "Failed to fetch release checksums for $latest_tag"
    exit 2
  fi
