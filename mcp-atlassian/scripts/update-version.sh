#!/usr/bin/env bash
# Appends the newest (or an explicit) mcp-atlassian PyPI release to
# releases.json as a new version-table entry (keyed by version) and sets
# .latest to it. Existing entries are preserved so consumers can still select
# past versions. The version data in flake.nix is never hand-edited.
#
# mcp-atlassian's sdist lives at a content-hash URL with an UNDERSCORE filename
# (mcp_atlassian-<ver>.tar.gz), so nixpkgs' fetchPypi — which guesses a legacy
# `source/m/<name>/<name>-<ver>.tar.gz` URL — 404s. This updater resolves the
# real sdist URL from the PyPI JSON API and stores it per-entry; flake.nix
# fetches that exact URL with fetchurl.
set -euo pipefail
...
readonly PYPI_API_URL="https://pypi.org/pypi/mcp-atlassian/json"
readonly BIN_NAME="mcp-atlassian"
...
# Resolve the real sdist URL for a version from the PyPI JSON (handles the
# content-hash path and the underscore filename that fetchPypi gets wrong).
get_package_url() {
  local version="$1"
  curl -fsSL "$PYPI_API_URL" \
    | jq -r --arg v "$version" \
        '.releases[$v][] | select(.packagetype=="sdist") | .url' \
    | head -n1
}
...
verify_build() {
  ...
  # Load-gate smoke test: --help exercises the full import chain (including
  # truststore injection at import time) without needing credentials or a live
  # Atlassian connection. A failure here means the package is broken.
  if ! timeout 30 "$out_path/bin/$BIN_NAME" --help >/dev/null 2>&1; then
    log_error "Binary failed its --help load-gate smoke test"
    return 1
  fi
...
Appends the newest (or an explicit) mcp-atlassian PyPI release to releases.json
as a new version-table entry (keyed by version) and sets .latest to it. The real
sdist URL (content-hash path, underscore filename) and its sha256 are resolved
from the PyPI JSON API. Existing entries are preserved so consumers can still
select past versions. The version data in flake.nix is never touched.

Options:
  --version VERSION   Append a specific version (default: latest)
  --check             Only check for updates (exit 1 if update available)
  --rehash            Recompute the source URL+hash for the current latest
...
Examples:
  ...
  ./scripts/update-version.sh --version 0.24.0
...
main() {
  ...
  log_info "Resolving sdist URL and hash for $new_version..."
  local package_url new_hash
  package_url="$(get_package_url "$new_version")"
  if [ -z "$package_url" ]; then
    log_error "Failed to resolve sdist URL for $new_version from PyPI JSON"
    exit 1
  fi
  ...
  if [ "$do_commit" = true ]; then
    ...
    if [ "$current_version" != "$new_version" ]; then
      ...
      msg="chore(${PACKAGE_DIR_NAME}): update version"
    fi
    maybe_git_commit "$msg" "releases.json"
  fi

  log_info "Successfully selected mcp-atlassian $new_version (was $current_version)"
}
