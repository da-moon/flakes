#!/usr/bin/env bash
# Appends the newest @upstash/context7-mcp npm release to releases.json (the
# JSON version table read by flake.nix) and sets it as .latest. Never
...
readonly PACKAGE_ATTR="context7-mcp-server"
...
tarball hash and generates+commits deps/<version>/{package.json,package-lock.json,.npmrc}.
