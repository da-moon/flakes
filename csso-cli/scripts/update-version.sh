#!/usr/bin/env bash
# Appends the newest (or an explicit) csso-cli npm release to releases.json (the
# JSON version table read by flake.nix) and sets it as .latest. csso-cli is a
# TAGGED npm package, so:
#   key     = the npm version (e.g. "4.0.2")
#   version = the same npm version
#
# Two kinds of fixed-output hashes are (re)computed via jq — flake.nix is never
# hand-edited:
#   - .hash                 : npm tarball (.tgz) fetchurl hash (single, portable),
#                             obtained via `nix store prefetch-file`.
#   - .outputHashBySystem.* : per-system npm-deps FOD hash, recomputed from
#                             scratch via the fakeHash -> nix build -> parse
#                             "got:" method (only the build system is refreshed).
set -euo pipefail
...
get_latest_version_from_npm() {
  ...
  printf '%s' "$latest_json" | jq -r '.version // empty'
}
...
Appends the newest (or an explicit) csso-cli npm release to releases.json as a
new version-table entry (keyed by version) and sets .latest to it. Recomputes
the npm tarball hash and the per-system npm-deps FOD hash via jq — the version
data in flake.nix is never touched. Existing entries are preserved so consumers
can still select past versions.
...
Examples:
  ...
  ./scripts/update-version.sh --version 4.0.2
EOF
}
...
main() {
  ...
  local key="$latest_version"
  local attr
  attr="${PNAME}_$(sanitize_key "$key")"

  # 1) npm tarball hash (portable across systems, prefetched directly).
  local tarball_url
  ...
  # Seed/upsert the entry with the real tarball hash and a fakeHash for the
  # build system's outputHash so nix reveals the real one on build.
  local tmp
  tmp="$(mktemp)"
  jq --arg k "$key" \
  ...
  # 2) per-system npm-deps FOD outputHash (for the build system).
  ...
  if [ -z "$out_hash" ]; then
    ...
    tmp="$(mktemp)"
    jq --arg k "$key" --arg bsys "$BUILD_SYSTEM" --arg h "$out_hash" \
