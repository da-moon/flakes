#!/usr/bin/env bash
# Appends the newest (or an explicit) amber-lsp release to releases.json (the
# JSON version table read by flake.nix) and sets it as .latest. amber-lsp
# ships TAGGED GitHub releases with per-target prebuilt tarballs, so:
#   key     = the upstream version (e.g. "0.3.0")
...
# Upstream publishes no aarch64-linux asset, so only the three remaining
...
readonly REPO_OWNER="amber-lang"
readonly REPO_NAME="amber-lsp"
# Upstream release-asset target triples keyed by nix system.
...
verify_build() {
  ...
  local out_path
  if ! out_path="$(cd "$pkg_dir" && nix build ".#amber-lsp_${sanitized_key}" --no-link --print-out-paths --no-write-lock-file)"; then
    log_error "nix build failed for amber-lsp_${sanitized_key}"
    return 1
  fi
  if [ -z "$out_path" ] || [ ! -x "$out_path/bin/amber-lsp" ]; then
    log_error "Build succeeded but expected binary not found at: $out_path/bin/amber-lsp"
    return 1
  fi
  ...
  timeout 30 "$out_path/bin/amber-lsp" --version >/dev/null 2>&1 || true
...
Appends the newest (or an explicit) amber-lsp release to releases.json as a
new version-table entry (keyed by version) and sets .latest to it. Existing
entries are preserved so consumers can still select past versions. Per-target
tarball hashes are prefetched and written via jq — flake.nix is never
touched.
...
Examples:
  ...
  ./scripts/update-version.sh --version 0.3.0
EOF
}
...
main() {
  ...
  # Prefetch per-target tarball hashes from the release assets.
  ...
  for system in "${!SYSTEM_TO_RELEASE_SUFFIX[@]}"; do
    ...
    url="https://github.com/$REPO_OWNER/$REPO_NAME/releases/download/$latest_tag/amber-lsp-${suffix}.tar.gz"
  ...
  log_info "Successfully appended amber-lsp $latest_version (latest was $current_version)"
}
