{
  ...
  outputs =
    ...
    let
      ...
      telegramUserId = builtins.fromJSON (builtins.readFile "/home/${user}/.secrets/telegram-user-id");
      ...
      # The Linux configuration above intentionally keeps its WSL-oriented
      # systemd, Chromium, and userspace Tailscale services.  On Apple Silicon,
      # let nix-openclaw and Home Manager provide the native launchd services.
      mkDarwinHome =
        {
          username,
          homeDirectory ? "/Users/${username}",
          telegramUserId ? builtins.fromJSON (builtins.readFile "${homeDirectory}/.secrets/telegram-user-id"),
        }:
        let
          darwinSystem = "aarch64-darwin";
          darwinPkgs = import nixpkgs { system = darwinSystem; };
          darwinPkgsUnstable = import nixpkgs-unstable { system = darwinSystem; };
          darwinRuntimePackages = [
            inputs.claude-code.packages.${darwinSystem}.claude-code
            inputs.codex.packages.${darwinSystem}.codex
            inputs.beads.packages.${darwinSystem}.beads
            ...
            (
              { lib, ... }:
              {
                home = {
                  ...
                  stateVersion = "24.11";
                  packages = darwinRuntimePackages ++ [
                    darwinPkgs.which
                    darwinPkgs.unzip
                    darwinPkgsUnstable.helix
                    darwinPkgsUnstable.ripgrep
                    darwinPkgsUnstable.fd
                  ];
                };

                programs.home-manager.enable = true;

                home.activation.openclawWorkspaceDirs = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
                  $DRY_RUN_CMD ${darwinPkgs.coreutils}/bin/install -d -m 0755 \
                    ${lib.escapeShellArg "${homeDirectory}/.openclaw/workspace/memory"} \
                    ${lib.escapeShellArg "${homeDirectory}/.openclaw/workspace/memory/cron"}
                '';

                services.syncthing.enable = true;

                programs.openclaw = rec {
                  enable = true;
                  package = inputs.nix-openclaw.packages.${darwinSystem}.openclaw;
                  runtimePackages = darwinRuntimePackages;
                  systemd.enable = false;
                  launchd.enable = true;
                  # The gateway CLI and launchd agent are supported here.  The
                  # current upstream app-defaults activation path fails before
                  # building, so leave the optional .app bundle unmanaged.
                  installApp = false;
                  # The upstream goplaces subflake currently carries a mutable
                  # path input in its lock file, which builtins.getFlake rejects.
                  bundledPlugins.goplaces.enable = false;

                  # nix-openclaw reads file-valued variables at runtime, so
                  # secrets stay outside the Nix store and work with launchd.
                  environment = {
                    OPENCLAW_GATEWAY_TOKEN = "${homeDirectory}/.secrets/openclaw-gateway-token";
                    OPENCLAW_GATEWAY_PASSWORD = "${homeDirectory}/.secrets/openclaw-gateway-password";
                    ANTHROPIC_API_KEY = "${homeDirectory}/.secrets/anthropic-api-key";
                    OPENAI_API_KEY = "${homeDirectory}/.secrets/openai-api-key";
                    MOONSHOT_API_KEY = "${homeDirectory}/.secrets/moonshot-api-key";
                    MINIMAX_API_KEY = "${homeDirectory}/.secrets/minimax-api-key";
                    ZAI_API_KEY = "${homeDirectory}/.secrets/zai-api-key";
                    PERPLEXITY_API_KEY = "${homeDirectory}/.secrets/perplexity-api-key";
                    VOYAGE_API_KEY = "${homeDirectory}/.secrets/voyage-api-key";
                  };
                  ...
                  config = {
                    ...
                    channels.telegram = {
                      accounts.default.tokenFile = "${homeDirectory}/.secrets/telegram-bot-token";
                      allowFrom = [ telegramUserId ];
                  ...
                  # Defining the instance explicitly uses the complete macOS
                  # option defaults from nix-openclaw (including nixMode).
                  instances.default = {
                    inherit
                      package
                      runtimePackages
    ...
    in
    {
      lib = {
        ...
        inherit mkDarwinHome;
      };

      homeConfigurations."${user}" =
        if system == "aarch64-darwin" then
          mkDarwinHome { username = user; }
        else if system == "x86_64-linux" then
          mkHome user
        else
          throw "openclaw supports x86_64-linux and aarch64-darwin";
    };
}
# ============================================================================
...
# 3. Runtime secrets
#
#    Linux: create ~/.secrets/openclaw-env with KEY=VALUE lines:
...
#
#    macOS: launchd reads one plain-text file per secret (value only):
#      ~/.secrets/openclaw-gateway-token
#      ~/.secrets/openclaw-gateway-password
#      ~/.secrets/anthropic-api-key
#      ~/.secrets/openai-api-key
#      ~/.secrets/moonshot-api-key
#      ~/.secrets/minimax-api-key
#      ~/.secrets/zai-api-key
#      ~/.secrets/perplexity-api-key
#      ~/.secrets/voyage-api-key
#
...
#
# The remaining service-management steps are Linux-only.  On macOS, Home
# Manager installs the OpenClaw and Syncthing launchd agents automatically.
#
