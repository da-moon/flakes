...
readonly NPM_PACKAGE="agent-browser"
readonly TARBALL_NAME="agent-browser"
readonly PACKAGE_ATTR="agent-browser"
readonly BIN_NAME="agent-browser"
...
get_current_version() {
  sed -n 's/^[[:space:]]*version = "\([^"]*\)".*/\1/p' "$flake_file" | head -n1 || true
}
...
set_output_hash_placeholder_for_system() {
  ...
  if ! grep -Eq "\"${system_key}\"[[:space:]]*=[[:space:]]*\"${placeholder}\";" "$flake_file"; then
...
update_output_hash_for_system() {
  ...
  if ! grep -Fq "\"${new_hash_value}\";" "$flake_file"; then
...
main() {
  ...
  if [ "$check_only" = true ]; then
    ...
    if [ "$needs_rehash" = true ]; then
      log_warn "Rehash required for current system"
      exit 1
    fi
    exit 0
  fi
