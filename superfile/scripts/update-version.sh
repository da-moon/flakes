#!/usr/bin/env bash
# Appends the newest (or an explicit) superfile release to releases.json as a
# new version-table entry (keyed by version, since upstream is tagged) and sets
# .latest to it. Existing entries are preserved so consumers can still select
# past versions. The version data in flake.nix is never touched.
set -euo pipefail
...
readonly REPO_OWNER="yorukot"
readonly REPO_NAME="superfile"

# Systems we ship prebuilt binaries for. Upstream publishes x86_64 Linux and
# Apple-silicon macOS archives; values are "<os>-<arch>" as used in asset names
...
# Asset names look like superfile-linux-v1.6.0-amd64.tar.gz.
asset_name() {
  local version="$1"
  local target="$2"
  local os="${target%%-*}"
  local arch="${target##*-}"
  printf 'superfile-%s-v%s-%s.tar.gz\n' "$os" "$version" "$arch"
}
...
asset_url() {
  ...
  printf 'https://github.com/%s/%s/releases/download/v%s/%s\n' \
    "$REPO_OWNER" "$REPO_NAME" "$version" "$(asset_name "$version" "$target")"
}
...
verify_build() {
  ...
  local out_path
  if ! out_path="$(cd "$pkg_dir" && nix build ".#superfile_${sanitized_key}" --no-link --print-out-paths --no-write-lock-file)"; then
    log_error "nix build failed for superfile_${sanitized_key}"
    return 1
  fi
  if [ -z "$out_path" ] || [ ! -x "$out_path/bin/spf" ]; then
    log_error "Build succeeded but expected binary not found at: $out_path/bin/spf"
    return 1
  fi
  ...
  timeout 30 "$out_path/bin/spf" --version >/dev/null 2>&1 || true
...
Appends the newest (or an explicit) superfile release to releases.json as a new
...
Examples:
  ...
  ./scripts/update-version.sh --version 1.6.0
EOF
}
...
main() {
  ...
  # Compute per-arch SRI hashes by prefetching each release tarball.
  local system_key target hash sri_hash
  local hashes_json="{}"
  for system_key in "${!ASSET_NAME_BY_SYSTEM[@]}"; do
    target="${ASSET_NAME_BY_SYSTEM[$system_key]}"
    log_info "Prefetching $(asset_name "$latest_version" "$target") ..."
    if ! sri_hash="$(prefetch_sha256_sri "$(asset_url "$latest_version" "$target")")" || [ -z "$sri_hash" ]; then
      log_error "Failed to prefetch hash for ${system_key}"
      exit 2
    fi
    log_info "${system_key} hash: ${sri_hash}"
    hashes_json="$(jq -n --argjson h "$hashes_json" --arg s "$system_key" --arg v "$sri_hash" \
      '$h + {($s): $v}')"
  done
  ...
  log_info "Successfully appended superfile $latest_version (latest was $current_version)"
}
