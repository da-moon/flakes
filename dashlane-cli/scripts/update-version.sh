#!/usr/bin/env bash
# Appends the newest upstream release of Dashlane/dashlane-cli as a new entry in
# releases.json (the JSON version table the flake reads) and sets it as .latest.
# Dashlane CLI ships TAGGED GitHub releases, so:
#   key     = the release version (e.g. "6.2628.1")
#   version = same
# Per-arch prebuilt-binary SRI hashes are prefetched from the raw release assets
# named dcli-linux-x64, dcli-macos-x64, and dcli-macos-arm64. The version data
...
readonly REPO_OWNER="Dashlane"
readonly REPO_NAME="dashlane-cli"

# Map nix system -> Dashlane release-asset file name.
...
readonly SYSTEM_KEYS=(x86_64-linux aarch64-darwin x86_64-darwin)
...
verify_build() {
  ...
  local out_path
  if ! out_path="$(cd "$pkg_dir" && nix build ".#dashlane-cli_${sanitized_key}" --no-write-lock-file --no-link --print-out-paths)"; then
    log_error "nix build failed for dashlane-cli_${sanitized_key}"
    return 1
  fi
  if [ -z "$out_path" ] || [ ! -x "$out_path/bin/dcli" ]; then
    log_error "Build succeeded but expected binary not found at: $out_path/bin/dcli"
    return 1
  fi
  ...
  # dcli may prompt for a device if run without arguments, so only --help is safe.
  timeout 10 "$out_path/bin/dcli" --help >/dev/null 2>&1 || true
...
Appends the newest (or an explicit) Dashlane CLI release to releases.json as a
new version-table entry (keyed by version) and sets .latest to it. Existing
entries are preserved so consumers can still select past versions. Per-arch
prebuilt binary hashes are recomputed via jq — flake.nix is never touched.
...
Examples:
  ...
  ./scripts/update-version.sh --version 6.2628.1
EOF
}
...
main() {
  ...
  local hashes_json="{}"
  local key asset url hash
  for key in "${SYSTEM_KEYS[@]}"; do
    asset="${ASSET_BY_SYSTEM[$key]}"
  ...
  log_info "Successfully appended dashlane-cli $latest_version (latest was $current_version)"
}
