...
verify_build() {
  ...
  local out_path
  if ! out_path="$(cd "$pkg_dir" && nix build ".#claude-code_${sanitized_key}" --no-link --print-out-paths --no-write-lock-file)"; then
    log_error "nix build failed for claude-code_${sanitized_key}"
    return 1
  fi
  ...
  timeout 30 "$out_path/bin/claude" --version >/dev/null 2>&1 || true
...
Appends the newest (or an explicit) claude-code release to releases.json as a
...
main() {
  ...
  log_info "Successfully appended claude-code $latest_version (latest was $current_version)"
}
