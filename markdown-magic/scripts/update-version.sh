#!/usr/bin/env bash
# Appends the newest markdown-magic npm release to releases.json (the JSON
# version table read by flake.nix) and sets it as .latest. Never hand-edits
...
#   - deps/<v>/{package.json,package-lock.json} : the committed, pinned
#     lockfile (devDependencies stripped, so unbuildable dev-only packages
#     are never fetched). No .npmrc is needed: plain
#     `npm install --package-lock-only` reproduces the committed lockfile.
set -euo pipefail
...
# stripped, so importNpmLock's `npm ci` installs a lean prod tree. No .npmrc
# is written: plain --package-lock-only reproduces the committed lockfile.
generate_npm_lock() {
  ...
  rm -rf "$work"
  log_info "  committed deps/${version}/{package.json,package-lock.json}"
}
...
releases.json and sets it as .latest. For the new version it prefetches the
tarball hash and generates+commits deps/<version>/{package.json,package-lock.json}.
