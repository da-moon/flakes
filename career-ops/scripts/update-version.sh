...
update_version_rev_hash() {
  local version="$1" tag="$2" hash="$3"
  python3 - "$flake_file" "$version" "$tag" "$hash" <<'PY'
import re
import sys
from pathlib import Path
path = Path(sys.argv[1])
version, tag, source_hash = sys.argv[2:]
text = path.read_text()
text = re.sub(r'(version = ")[^"]+(";)', rf'\g<1>{version}\2', text, count=1)
text = re.sub(r'(rev = ")[^"]+(";)', rf'\g<1>{tag}\2', text, count=1)
...
main() {
  ...
  local current tag version source_hash
