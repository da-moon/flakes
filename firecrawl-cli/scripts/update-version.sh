#!/usr/bin/env bash
# Appends the newest firecrawl-cli npm release to releases.json (the JSON
# version table read by flake.nix) and sets it as .latest. Never hand-edits the
# version data in flake.nix.
#
# firecrawl-cli ships tagged npm releases, so:
#   key     = the npm version (e.g. "1.19.27")
...
# nixpkgs ref used to obtain node+corepack (pnpm itself is not fetched from
# here — see generate_pnpm_lock: the exact pnpm version is pinned by each
# release tarball's own package.json "packageManager" field, via corepack,
# so the generated lockfile reproduces byte-for-byte regardless of which
# pnpm happens to be current in nixpkgs).
...
# node + corepack from the pinned nixpkgs (used only as a runtime to invoke
# the version-pinned pnpm below — nixpkgs' own pnpm derivation is NOT used
# here, since its patch version floats independently of what generated the
# already-committed lockfiles and does not reproduce them byte-for-byte).
node_run() { nix shell "${NIXPKGS_REF}#nodejs_22" --command "$@"; }
...
# package.json so pnpm.fetchDeps' --frozen-lockfile check passes at build time.
#
# Crucially, this uses the EXACT pnpm version pinned by the tarball's own
# package.json "packageManager" field (via corepack), not whatever pnpm major
# nixpkgs currently ships: pnpm's lockfile writer has changed what metadata it
# emits (e.g. an added "libc:" field on optional native deps) across patch
# releases, so only the pinned version reproduces the committed lockfile
# byte-for-byte. corepack downloads that exact pnpm build on demand and never
# touches the tarball's package.json (no "corepack use").
generate_pnpm_lock() {
  ...
  (
    ...
    pm_spec="$(node_run node -e "console.log(require('./package.json').packageManager || '')")"
    pm_version="$(printf '%s' "$pm_spec" | sed -E 's/^pnpm@([0-9][0-9.]*).*/\1/')"
    [ -n "$pm_version" ] || { log_error "no packageManager pnpm version pinned in package.json"; exit 1; }
    node_run corepack "pnpm@${pm_version}" install --lockfile-only --ignore-scripts
  )
