...
get_latest_version_from_npm() {
  ...
  local version_field
  version_field="$(printf '%s\n' "$latest_json" | grep -o '"version":[[:space:]]*"[^"]*"' | head -n1 || true)"
  printf '%s\n' "$version_field" \
...
update_flake_version() {
  ...
  if ! grep -Fq "version = \"${new_version}\";" "$flake_file"; then
    log_error "Failed to update package version in flake.nix"
    return 1
  fi
}
...
update_tarball_hash() {
  ...
  if ! grep -Fq "hash = \"${new_hash}\";" "$flake_file"; then
    log_error "Failed to update tarball hash in flake.nix"
    return 1
  fi
}
...
Examples:
  ...
  ./scripts/update-version.sh --version 1.4.1
EOF
}
