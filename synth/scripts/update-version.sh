#!/usr/bin/env bash
# Appends the newest (or an explicit) shuttle-hq/synth release to releases.json
# (the JSON version table read by flake.nix) and sets it as .latest. synth ships
# TAGGED GitHub releases with per-target prebuilt tarballs, so:
...
readonly REPO_OWNER="shuttle-hq"
readonly REPO_NAME="synth"
# Upstream release-asset file names keyed by nix system.
declare -Ar SYSTEM_TO_ASSET=(
...
verify_build() {
  ...
  local out_path
  if ! out_path="$(cd "$pkg_dir" && nix build ".#synth_${sanitized_key}" --no-link --print-out-paths --no-write-lock-file)"; then
    log_error "nix build failed for synth_${sanitized_key}"
    return 1
  fi
  if [ -z "$out_path" ] || [ ! -x "$out_path/bin/synth" ]; then
    log_error "Build succeeded but expected binary not found at: $out_path/bin/synth"
    return 1
  fi
  ...
  timeout 30 "$out_path/bin/synth" --version >/dev/null 2>&1 || true
...
Examples:
  ...
  ./scripts/update-version.sh --version 0.6.9
EOF
}
...
main() {
  ...
  for system in "${SYSTEM_KEYS[@]}"; do
    ...
    if [ -z "$sri_hash" ]; then
      log_error "Failed to prefetch hash for $asset ($system): $url"
      exit 2
    fi
